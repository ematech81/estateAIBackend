...
describe('POST /api/leads', () => {
    it('creates a lead against a real listing without auth', async () => {
        ...
        const res = await (0, supertest_1.default)(app).post('/api/leads').send({
...
describe('GET /api/leads/mine', () => {
    it('requires auth', async () => {
        const res = await (0, supertest_1.default)(app).get('/api/leads/mine');
    ...
    it("only returns leads for the requesting agent's own listings", async () => {
        ...
        const res = await (0, supertest_1.default)(app).get('/api/leads/mine').set('Authorization', `Bearer ${agentA}`);
...
describe('PATCH /api/leads/:id', () => {
    it('rejects a status update from an agent who does not own the lead', async () => {
        ...
        const leadRes = await (0, supertest_1.default)(app)
            .post('/api/leads')
