...
describe('GET /api/users/me', () => {
    it('requires auth', async () => {
        const res = await (0, supertest_1.default)(app).get('/api/users/me');
        expect(res.status).toBe(401);
    });
    it('returns the authenticated user without the password hash', async () => {
        const token = await registerAndLogin();
        const res = await (0, supertest_1.default)(app).get('/api/users/me').set('Authorization', `Bearer ${token}`);
...
describe('PATCH /api/users/me/password', () => {
    ...
    it('updates the password and lets the user log in with the new one', async () => {
        ...
        const changeRes = await (0, supertest_1.default)(app)
            .patch('/api/users/me/password')
            .set('Authorization', `Bearer ${token}`)
            .send({ currentPassword: 'supersecret123', newPassword: 'newpassword456' });
        expect(changeRes.status).toBe(200);
        const oldLogin = await (0, supertest_1.default)(app).post('/api/auth/login').send({ email, password: 'supersecret123' });
        expect(oldLogin.status).toBe(401);
        const newLogin = await (0, supertest_1.default)(app).post('/api/auth/login').send({ email, password: 'newpassword456' });
...
describe('DELETE /api/users/me', () => {
    ...
    it('deletes the account and cascades its listings and leads', async () => {
        ...
        const res = await (0, supertest_1.default)(app).delete('/api/users/me').set('Authorization', `Bearer ${token}`);
        expect(res.status).toBe(204);
        expect(await User_1.User.findById(userId)).toBeNull();
        expect(await Property_1.Property.countDocuments({ createdBy: userId })).toBe(0);
        expect(await Lead_1.Lead.countDocuments({ agent: userId })).toBe(0);
    });
});
...
describe('User deletion cascade (models/User.ts middleware)', () => {
    ...
    it('findByIdAndDelete removes the user’s listings and leads too', async () => {
        const token = await registerAndLogin();
        const meRes = await (0, supertest_1.default)(app).get('/api/users/me').set('Authorization', `Bearer ${token}`);
        ...
        expect(await Property_1.Property.countDocuments({ createdBy: userId })).toBe(1);
        expect(await Lead_1.Lead.countDocuments({ agent: userId })).toBe(1);
    ...
    it('does not touch another user’s listings', async () => {
        ...
        const meB = await (0, supertest_1.default)(app).get('/api/users/me').set('Authorization', `Bearer ${tokenB}`);
        ...
        await User_1.User.findByIdAndDelete(userBId);
        expect(await Property_1.Property.countDocuments({})).toBe(1);
    });
});
const validListingPayload = {
    title: 'Newly built 3 bedroom duplex, Lekki Phase 1',
...
describe('GET /api/users/agents', () => {
    it('is public and lists registered agents without email/phone', async () => {
        const token = await registerAndLogin();
        const res = await (0, supertest_1.default)(app).get('/api/users/agents');
        expect(res.status).toBe(200);
    ...
    it('excludes admin accounts', async () => {
        ...
        expect(res.body.agents.some((a) => a.name === 'Admin')).toBe(false);
    });
...
describe('GET /api/users/agents/:id', () => {
    it('returns a single public agent profile', async () => {
        const token = await registerAndLogin();
        const me = await (0, supertest_1.default)(app).get('/api/users/me').set('Authorization', `Bearer ${token}`);
        const res = await (0, supertest_1.default)(app).get(`/api/users/agents/${me.body.user.id}`);
        expect(res.status).toBe(200);
    ...
    it('404s for a malformed id instead of 500ing', async () => {
        const res = await (0, supertest_1.default)(app).get('/api/users/agents/not-a-valid-id');
    ...
    it('404s for an admin account (not a public agent)', async () => {
        ...
        const res = await (0, supertest_1.default)(app).get(`/api/users/agents/${admin._id}`);
        expect(res.status).toBe(404);
    });
});
describe('GET /api/listings?agentId=', () => {
    it('filters to only that agent’s listings', async () => {
        const tokenA = await registerAndLogin('agent-a@example.com');
        const tokenB = await registerAndLogin('agent-b@example.com');
        const meA = await (0, supertest_1.default)(app).get('/api/users/me').set('Authorization', `Bearer ${tokenA}`);
        await (0, supertest_1.default)(app).post('/api/listings').set('Authorization', `Bearer ${tokenA}`).send(validListingPayload);
        await (0, supertest_1.default)(app).post('/api/listings').set('Authorization', `Bearer ${tokenB}`).send(validListingPayload);
        const res = await (0, supertest_1.default)(app).get(`/api/listings?agentId=${meA.body.user.id}`);
        expect(res.body.total).toBe(1);
    });
});
