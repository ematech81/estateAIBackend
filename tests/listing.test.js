...
const extraction_service_1 = require("../src/services/ai/extraction.service");
...
const mockedSendNewListingEmail = emailService.sendNewListingEmail;
...
describe('POST /api/listings/draft', () => {
    it('requires auth', async () => {
        const res = await (0, supertest_1.default)(app).post('/api/listings/draft').send({ text: 'a'.repeat(20) });
    ...
    it('returns a structured draft from a mocked AI provider, unmodified', async () => {
        ...
        (0, extraction_service_1.setAIProvider)(mockProvider);
...
describe('GET /api/listings (search/pagination)', () => {
    ...
    async function createNListings(token, n, email = 'agent@example.com') {
        await User_1.User.updateOne({ email }, { planTier: 'premium', planExpiresAt: new Date(Date.now() + 30 * 86_400_000) });
        for (let i = 0; i < n; i++) {
    ...
    it('defaults to 12 per page and reports pagination metadata', async () => {
        ...
        const res = await (0, supertest_1.default)(app).get('/api/listings');
        expect(res.status).toBe(200);
    ...
    it('returns the remainder on page 2, with no overlap with page 1', async () => {
        const token = await registerAndLogin();
        await createNListings(token, 15);
        const page1 = await (0, supertest_1.default)(app).get('/api/listings?page=1');
        const page2 = await (0, supertest_1.default)(app).get('/api/listings?page=2');
        expect(page2.body.listings).toHaveLength(3);
        expect(page2.body.page).toBe(2);
        const page1Ids = page1.body.listings.map((l) => l._id);
        const page2Ids = page2.body.listings.map((l) => l._id);
        expect(page1Ids.some((id) => page2Ids.includes(id))).toBe(false);
    });
    it('an empty-of-results page still reports correct totals, not an error', async () => {
        const token = await registerAndLogin();
        await createNListings(token, 5);
        const res = await (0, supertest_1.default)(app).get('/api/listings?page=99');
        expect(res.status).toBe(200);
    ...
    it('filters by international (location.country !== Nigeria)', async () => {
        const token = await registerAndLogin();
        await (0, supertest_1.default)(app).post('/api/listings').set('Authorization', `Bearer ${token}`).send(validListingPayload); // Nigeria
        await (0, supertest_1.default)(app)
            .post('/api/listings')
            .set('Authorization', `Bearer ${token}`)
            .send({
        ...
        const noFilter = await (0, supertest_1.default)(app).get('/api/listings');
        const internationalOnly = await (0, supertest_1.default)(app).get('/api/listings?international=true');
        const localOnly = await (0, supertest_1.default)(app).get('/api/listings?international=false');
    ...
    it('filters by propertyType', async () => {
        ...
        const res = await (0, supertest_1.default)(app).get('/api/listings?propertyType=land');
    ...
    it('filters by min/max price range', async () => {
        ...
        const res = await (0, supertest_1.default)(app).get('/api/listings?minPrice=5000000&maxPrice=15000000');
        expect(res.body.total).toBe(1);
...
describe('New-listing email notification (Premium plan only)', () => {
    ...
    it('does not email anyone when a Free-plan agent publishes', async () => {
        const token = await registerAndLogin();
        await User_1.User.updateOne({ email: 'agent@example.com' }, { emailMarketingOptIn: false });
        await (0, supertest_1.default)(app).post('/api/listings').set('Authorization', `Bearer ${token}`).send(validListingPayload);
        await flush();
    ...
    it('emails opted-in users when a Premium-plan agent publishes', async () => {
        ...
        await User_1.User.updateOne({ email: 'premium-poster@example.com' }, { planTier: 'premium', planExpiresAt: new Date(Date.now() + 30 * 86_400_000) });
        await registerAndLogin('opted-in@example.com');
        await User_1.User.updateOne({ email: 'opted-in@example.com' }, { emailMarketingOptIn: true });
    ...
    it('never emails the poster themselves, even if they are opted in', async () => {
        ...
        await User_1.User.updateOne({ email: 'premium-poster2@example.com' }, { planTier: 'premium', planExpiresAt: new Date(Date.now() + 30 * 86_400_000), emailMarketingOptIn: true });
        await (0, supertest_1.default)(app).post('/api/listings').set('Authorization', `Bearer ${posterToken}`).send(validListingPayload);
        await flush();
        expect(mockedSendNewListingEmail).not.toHaveBeenCalled();
    });
});
describe('Listing quota (Free plan default)', () => {
    it('allows up to the Free plan\'s 2-per-month limit', async () => {
        const token = await registerAndLogin();
        const first = await (0, supertest_1.default)(app).post('/api/listings').set('Authorization', `Bearer ${token}`).send(validListingPayload);
        const second = await (0, supertest_1.default)(app).post('/api/listings').set('Authorization', `Bearer ${token}`).send(validListingPayload);
    ...
    it('blocks a 3rd listing in the same month with a clear upgrade message', async () => {
        ...
        await (0, supertest_1.default)(app).post('/api/listings').set('Authorization', `Bearer ${token}`).send(validListingPayload);
        await (0, supertest_1.default)(app).post('/api/listings').set('Authorization', `Bearer ${token}`).send(validListingPayload);
        const third = await (0, supertest_1.default)(app).post('/api/listings').set('Authorization', `Bearer ${token}`).send(validListingPayload);
        expect(third.status).toBe(403);
        expect(third.body.message).toMatch(/upgrade/i);
    });
    it('an active Premium plan is not capped', async () => {
        const token = await registerAndLogin();
        await User_1.User.updateOne({ email: 'agent@example.com' }, { planTier: 'premium', planExpiresAt: new Date(Date.now() + 30 * 86_400_000) });
        for (let i = 0; i < 4; i++) {
            const res = await (0, supertest_1.default)(app)
                .post('/api/listings')
                .set('Authorization', `Bearer ${token}`)
    ...
    it('an expired Premium plan is lazily treated as Free — capped again', async () => {
        const token = await registerAndLogin();
        await User_1.User.updateOne({ email: 'agent@example.com' }, { planTier: 'premium', planExpiresAt: new Date(Date.now() - 1000) });
        await (0, supertest_1.default)(app).post('/api/listings').set('Authorization', `Bearer ${token}`).send(validListingPayload);
        await (0, supertest_1.default)(app).post('/api/listings').set('Authorization', `Bearer ${token}`).send(validListingPayload);
        const third = await (0, supertest_1.default)(app).post('/api/listings').set('Authorization', `Bearer ${token}`).send(validListingPayload);
        expect(third.status).toBe(403);
...
describe('Search priority (paid plans rank first)', () => {
    it('ranks an active Premium listing above a Free listing, regardless of recency', async () => {
        ...
        await User_1.User.updateOne({ email: 'premium-agent@example.com' }, { planTier: 'premium', planExpiresAt: new Date(Date.now() + 30 * 86_400_000) });
    ...
    it('does not let an expired paid plan jump the queue', async () => {
        ...
        await User_1.User.updateOne({ email: 'lapsed-agent@example.com' }, { planTier: 'premium', planExpiresAt: new Date(Date.now() - 1000) });
        await (0, supertest_1.default)(app)
            .post('/api/listings')
...
describe('GET /api/listings/mine', () => {
    it('only returns listings created by the requesting agent', async () => {
        ...
        const res = await (0, supertest_1.default)(app).get('/api/listings/mine').set('Authorization', `Bearer ${tokenA}`);
        expect(res.status).toBe(200);
...
describe('GET /api/listings/mine (routing order regression)', () => {
    it('still requires auth and is not shadowed by the GET /:id route', async () => {
        ...
        const res = await (0, supertest_1.default)(app).get('/api/listings/mine');
        expect(res.status).toBe(401);
    });
});
describe('GET /api/listings/:id', () => {
    it('returns a public listing without leaking createdBy', async () => {
        ...
        const res = await (0, supertest_1.default)(app).get(`/api/listings/${listingId}`);
        expect(res.status).toBe(200);
    ...
    it('404s for a well-formed id that does not exist', async () => {
        const res = await (0, supertest_1.default)(app).get('/api/listings/64b7f9f9f9f9f9f9f9f9f9f9');
        expect(res.status).toBe(404);
    });
    it('404s for a malformed id instead of 500ing', async () => {
        const res = await (0, supertest_1.default)(app).get('/api/listings/not-a-valid-object-id');
        expect(res.status).toBe(404);
    });
});
describe('PATCH /api/listings/:id', () => {
    ...
    it('lets the owner update their own listing, persisting the change', async () => {
        ...
        const fetchRes = await (0, supertest_1.default)(app).get(`/api/listings/${listingId}`);
